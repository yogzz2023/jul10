...
def jpda(clusters, targets, kalman_filter):
...
for group in measurement_groups:
    for i, (x, y, z, mt) in enumerate(group):
        ...
        if not kalman_filter.first_rep_flag:
            ...
            Z = np.array([[x], [y], [z]])
            if kalman_filter.gating(Z):
                x_prev, y_prev, z_prev, mt_prev = group[i-1]
                dt = mt - mt_prev
                vx = (x - x_prev) / dt
                vy = (y - y_prev) / dt
                vz = (z - z_prev) / dt
...
plt.scatter(filtered_values_csv[:, 0], r_list[0], label='Filtered range (track id 31)', color='red', marker='*')
...
plt.scatter(filtered_values_csv[:, 0], az_list[1], label='Filtered azimuth (track id 31)', color='red', marker='*')
plt.xlabel('Time', color='black')
...
plt.scatter(filtered_values_csv[:, 0], el_list[2], label='Filtered elevation (track id 31)', color='red', marker='*')
plt.xlabel('Time', color='black')
