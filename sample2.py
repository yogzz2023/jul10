...
class CVFilter:
    ...
    def initialize_filter_state(self, x, y, z, vx, vy, vz, time):
        ...
        self.Meas_Time = time
        self.Pf = np.eye(6) * 1000  # Initial state covariance
    ...
    def update_step(self, Z):
        ...
        self.Meas_Time = self.Meas_Time + 1  # Assuming measurements are sequentially time-indexed
...
for group in measurement_groups:
    for i, (x, y, z, mt) in enumerate(group):
        if i == 0:
            ...
            kalman_filter.Meas_Time = mt
        elif i == 1:
            # Second measurement
            kalman_filter.Z2 = np.array([[x], [y], [z]])
            dt = mt - kalman_filter.Meas_Time
            vx = (kalman_filter.Z2[0] - kalman_filter.Z1[0]) / dt
            vy = (kalman_filter.Z2[1] - kalman_filter.Z1[1]) / dt
            vz = (kalman_filter.Z2[2] - kalman_filter.Z1[2]) / dt
            kalman_filter.initialize_filter_state(x, y, z, vx, vy, vz, mt)
        else:
            # Predict and update from the third measurement onwards
            ...
            r, az, el = cart2sph(kalman_filter.Sf[0], kalman_filter.Sf[1], kalman_filter.Sf[2])
            time_list.append(mt)
            r_list.append(r)
            az_list.append(az)
            el_list.append(el)
...
plt.scatter(time_list, r_list, label='Filtered range', color='green', marker='o')
...
plt.figure(figsize=(12, 6))
plt.subplot(facecolor="white")
plt.scatter(time_list, az_list, label='Filtered azimuth', color='green', marker='*')
plt.xlabel('Time', color='black')
...
plt.figure(figsize=(12, 6))
plt.subplot(facecolor="white")
plt.scatter(time_list, el_list, label='Filtered elevation', color='green', marker='*')
plt.xlabel('Time', color='black')
